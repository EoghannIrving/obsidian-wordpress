...
class WpProfileModal extends AbstractModal {
  ...
  constructor(
    ...
    this.tokenGotRef = AppState.events.on(EventType.OAUTH2_TOKEN_GOT, async token => {
      this.profileData.wpComOAuth2Token = token as WordPressOAuth2Token | undefined;
      if (atIndex >= 0) {
        // if token is undefined, just remove it
        this.plugin.settings.profiles[atIndex].wpComOAuth2Token = token as WordPressOAuth2Token | undefined;
